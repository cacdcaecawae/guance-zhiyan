...
已接入管理员 JSONL 导入、Qdrant 向量与 SQLite 关键词混合检索、每轮默认 RAG 及原文片段引用。普通用户无需上传，文献库主页面及跨会话成果归档尚未接入。自动化测试验证受控模型与协议响应，不能代替真实服务或全库效果验收。

RAG 的配置、数据格式、导入、重建与备份见 [RAG 开发说明](docs/RAG.md)。向量服务未配置、空库或索引不完整时明确报错，不以普通回答或演示资料代替检索。
...
| `pnpm test:rag`                   | 对本机真实 Qdrant 验证导入、检索与重建，使用受控测试向量          |
...
浏览器测试先构建前端，默认使用 3001、3002 与 4173 端口。3001 被其他任务占用时可设置 `E2E_BACKEND_PORT=33301`，其余测试端口仍需空闲；不要停止其他任务的服务。`tests/support/` 中的身份与模型替身只由测试入口加载，应用没有演示模式或测试身份配置开关。本机入口测试另起正式服务进程，将真实 DSH 适配器指向受控协议服务，验证免登录、多轮、刷新恢复、流式与停止；这不代表真实 DeepSeek 联调已完成。CI 在 PR 与主分支 push 时运行同样的 `pnpm check`。
...
共享文献原文、版本、片段和全文索引也保存在 SQLite；Qdrant 向量数据保存在单独卷中，需一起备份恢复，或在恢复原文后重新建向量索引。管理员命令为 `pnpm rag import <文献.jsonl>` 和 `pnpm rag reindex`。
...
服务重启后恢复已保存历史；意外中断标为停止，后续生成不会重新激活旧回答。SSE 每次连接先发送快照，之后只发送变化，文本按追加片段传输；流式投影复用不变历史。工具参数和结果在对话与轨迹中均提供有截断标记的预览，原始记录仍保存在 JSONL。SSE 断开不自动取消后台任务，重新连接会读取当前状态；停止按钮取消对应会话。身份集成后部署时，反向代理需支持 SSE、关闭该接口缓冲并提供 HTTPS。多进程共享执行状态不在当前实现范围。
...
下一阶段：确定学校认证和源库接入；选择并验证真实 embedding 服务，以真实问题集评测召回、引用与并发；文献库页面接入列表 API；之后比较重排模型，再规划跨文献研究流程。
