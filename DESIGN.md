...
风格名为“公文”：白纸黑字、衬线正文，藏青负责一切可交互元素，朱红只做装饰（编号、当前页签、文件格式块、标志），与中国社会科学院大学的中国红相呼应。中性色接近纯中性、只带一点暖意，不用明显泛黄的纸色；深色正文不用纯白以免中文发粗；轨迹分类色取靛蓝、赭石、橄榄等矿物色。对话是多轮往下排的问答，只有文件预览画成“一页纸”，文件与对话一眼可分。
...
- 界面控件用系统无衬线栈（`font-sans`）；阅读内容用 `font-serif`：最终回答正文、文件预览、顶栏会话标题、欢迎标题与校名署名。衬线首选自托管的 Noto Serif SC Variable（`@fontsource-variable/noto-serif-sc`，按字符范围分块按需加载，各电脑效果一致）；衬线栈不列宋体（SimSun 无粗体），缺衬线字体时回落无衬线；执行过程、表格与界面说明仍用无衬线；`font-mono` 只用于路径、耗时、工具参数等技术值
- 不引用外部字体 CDN
...
| `text-ui-lg`      | 16px | 顶栏会话标题、回答内三级标题               |
| `text-ui-prose`   | 15px | 最终回答正文与输入框文字，行高 1.75        |
| `text-ui-base`    | 14px | 界面正文、按钮、问题气泡、面板标题（默认） |
...
- 可拖动的尺寸：侧栏宽度（208–400px）、文件预览宽度（320–760px）、输入框最小高度（32–480px），都用 `ResizeHandle`（指针拖动或方向键），作为界面偏好写 localStorage
...
- 基础控件默认 `rounded-lg`；放在圆角容器内时用 `rounded-md`（如主题切换分段、回答下方的操作按钮）
- 徽标与引用角标 `rounded-sm`
- 不用裸 `rounded`；`rounded-2xl` 只用于参照 Claude 的输入框外壳与问题气泡，框内发送按钮用 `rounded-lg`；`rounded-full` 只用于悬浮的圆形“回到底部”按钮；边缘贴合的抽屉不加圆角；文件预览的“纸”用 `rounded-sm`
...
焦点不丢：点击后会暂时不可用的按钮（发送、重新提问、新建研究）用 `aria-disabled` 而不是 `disabled`；会消失的控件（回到底部、关闭预览）把焦点交给相邻位置；新建研究后聚焦输入框。输入框高度按视口封顶，吸顶标题的滚动容器留出 `scroll-padding`。
...
- 右侧文件预览：点击会话文件卡片打开，宽屏（≥1024px）为可拖宽的右栏，窄屏为右侧抽屉；灰色桌面上一页纸，Markdown 按正文排版、CSV 为三线表、文本等宽显示；Word / Excel 等暂不支持预览时不画空白的纸，在桌面上居中显示格式块、文件名与说明，从顶栏下载。资料面板（文献片段）随真实文献库引用后续实现，当前不显示虚构片段
...
- 对话参照 Claude 左右排列：问题靠右，放在浅灰（`bg-tag`）圆角气泡中，无衬线、最宽 85%；回答靠左、无外框、不缩进，正文衬线；轮次之间只用留白分隔。
- 执行过程按 DSH 的汇总行与紧凑事件行排列，整体像页边批注：左侧细线、`text-ui-caption` 次要色；汇总按工具种类计数（如“联网搜索 2 次 · 生成文件 1 次”），另列失败次数（配警示图标），没有工具时写“执行过程”；汇总行与事件行的展开箭头紧跟文字。工具状态放在图标位（执行中转圈并显示文字、失败为红色警示图标），状态文字对读屏保留。思考与工具详情默认折叠，过程整体可收起；最终回答保持独立可见。
- 回答下方操作行：复制最终回答原文（剪贴板不可用时显示“复制失败”，不假装成功）、失败或停止后带文字的“重新提问”描边按钮。离开底部阅读时出现“回到底部”。
- 轨迹按真实轮次展示系统、用户、上下文、助手和工具事件，展开查看内容；轮次标题吸顶。“时长”切换记录时间与等宽事件顺序，“全部展开 / 全部收起”一次展开或收起所有轮次（轮次也可逐个展开，所以它是动作按钮而非开关），“调用”切换工具行显示。时间分布带 25/50/75% 刻度并显示总记录耗时；状态与耗时各占固定列，耗时用等宽数字。未发生的事件不展示，不叠加并行工具耗时。
...
- 输入框参照 Claude：单行圆角框，随内容增高，发送按钮在框内右侧；模型下拉（`ModelPicker`）在框外右下方：按平台分组列出全部 DeepSeek 模型，触发器显示“模型 · 平台”（两平台有同名模型），生成中禁用；缺少密钥的平台在分组标题和触发器中标出；选择随成功发送保存至会话。发送按钮为藏青，禁用时用 `bg-tag`，不靠半透明），生成中换成带文字的“停止”。输入框顶边的把手可拖动调整高度
- 界面不放防御性提示语（如“所选平台会接收历史”“需核对来源”）；未实现的页面本身仍写明“尚未实现”
- 连接与操作错误显示在顶栏下、对齐内容栏并配警示图标，两个视图都可见；请求失败保留草稿；失败或停止后“重新提问”追加一次新的提问，保留原始记录
- 文件生成成功后才出现在“会话文件”中：按可用宽度自动排列的文件卡片（每张至少 16rem），左侧朱红格式印块、文件名与大小，整卡点击在右侧预览（打开时藏青描边并标“预览中”），右侧下载图标按钮；卡片不加阴影，悬停换底色；文献库和成果归档未实现时明确说明
