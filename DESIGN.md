...
- 对话与输入区 `w-full max-w-3xl` 居中；轨迹铺满工作区；侧栏 `w-64`
...
- 工作区提供“对话 / 轨迹”页签，支持键盘左右切换。对话使用无边框正文，执行过程按 DSH 的汇总行、分隔线和紧凑事件行排列；最终回答保持独立可见。思考与工具详情默认折叠，过程整体可收起。
- 轨迹按真实轮次展示系统、用户、上下文、助手和工具事件，展开查看内容。“时长”切换记录时间与等宽事件顺序，“轮次”切换全部轮次展开，“调用”切换工具行显示。未发生的事件不展示，不叠加并行工具耗时。
