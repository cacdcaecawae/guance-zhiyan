...
- 输入框底行左侧是模型下拉（`ModelPicker`）：按平台分组列出全部 DeepSeek 模型，触发器显示“模型 · 平台”（两平台有同名模型），生成中禁用；缺少密钥的平台在分组标题和触发器中标出；选择随成功发送保存至会话。底行右侧是发送按钮（藏青，禁用时用 `bg-tag`，不靠半透明），生成中换成带文字的“停止”。输入框顶边的把手可拖动调整高度
- 界面不放防御性提示语（如“所选平台会接收历史”“需核对来源”）；未实现的页面本身仍写明“尚未实现”
- 请求失败保留草稿；失败或停止后“重新提问”追加一次新的提问，保留原始记录
- 文件生成成功后才出现在“会话文件”中：格式标签（朱红描边）、文件名（点击在右侧预览）、大小与“下载”链接；文献库和成果归档未实现时明确说明
