...
- 内容区宽度 `w-full max-w-2xl` 居中；侧栏 `w-64`，资料面板 `w-80`
...
```
左侧导航与会话列表 | 中央研究工作区 | 按需展开的右侧资料面板
```

- ≥768px：侧栏常驻，可通过顶部栏按钮折叠（偏好写 localStorage）；<768px：侧栏为左侧抽屉
- ≥1024px：资料面板为右侧固定面板；<1024px：为右侧抽屉，避免挤压正文
- 顶部栏 `h-12`：侧栏开关、会话标题（截断并带 `title`）、演示标注
...
- 演示模式下：所有模拟内容标注“演示模式 / 非真实检索结果”
- 演示模式下：加载态只表示“正在获取数据”，不伪装 Agent 执行步骤
