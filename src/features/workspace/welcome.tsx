...
export function Welcome({ onPick }: WelcomeProps) {
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-3 py-6 text-center">
      ...
      <ul aria-label="示例问题" className="mt-3 flex flex-wrap justify-center gap-2">
        {sampleQuestions.map((q) => (
          <li key={q} className="min-w-0">
            <button
              type="button"
              onClick={() => onPick(q)}
              className="rounded-md border border-card-border bg-card px-3 py-1.5 text-ui-caption text-foreground-subtle outline-none transition-colors hover:border-brand/40 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
            >
              {q}
            </button>
          </li>
